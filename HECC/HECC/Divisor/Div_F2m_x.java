//Divisor of Hyperelliptic curve over Binary field
...
public class Div_F2m_x extends Div
{
	public Div_F2m_x(HyperellipticCurve C)
	{
		this.C=C;
		this.u=F2m_x.I;
		this.v=F2m_x.O;
	...
	public Div_F2m_x(HyperellipticCurve C,Fq_x u,Fq_x v)
	{
		this.C=C;
		this.u=u;
		this.v=v;
	...
	***************************************************************************************************/
	...
	public Div_F2m_x (HyperellipticCurve C, Random rndSrc) {
		this.C = C;
		
		//STEP I
		Fq x[] = new F2m [this.C.genus];
		Fq y[] = new F2m [this.C.genus];
		for (int i=0; i<this.C.genus; i++) {
			...
			do {
				...
				f = C.f.eval(x[i]).mul(C.h.mul(C.h).eval(x[i]).inverse());
			} while (!((F2m)f).trace().isZero());
			if (!C.h.eval(x[i]).isZero()) {
				y[i] = C.h.eval(x[i]).mul(((F2m)f).half_trace());
			} else { // TODO check this
				y[i] = (F2m)C.f.eval(x[i]).pow(F2m.degree()-1);
			}
		}
		...
		for (int i=0; i<this.C.genus; i++) {
			...
			ux.set_coefficient(1,F2m.I);
			this.u = this.u.mul(ux);
		}
		
		// STEP III
		// x matrix : each element x_ij = (x_i)^j
		this.v = new F2m_x(this.C.genus);
		Fq M[][] = new F2m [this.C.genus][this.C.genus];
	...
	*****************************************************************************************/
	...
	{
		Div D= new Div_F2m_x(C);
		Div D1=new Div_F2m_x(C);
		Div D2=new Div_F2m_x(C);
		BigInteger k;

		if (n.compareTo(BigInteger.valueOf(0)) == 0)
			return new Div_F2m_x(C);
